-- ============================================================
-- FASE 16 — corrige record_item_payment (Fase 11: pago por consumo/
-- persona) para el modelo de pago independiente de la Fase 12.
--
-- Desde Fase 12, orders.status ya no incluye 'pagado' (quedó solo para
-- el estado de preparación); el cobro se marca en orders.payment_status
-- ('pending'/'paid'). pay_order ya se actualizó para eso, pero
-- record_item_payment (pago dividido por persona) seguía escribiendo
-- status='pagado' — un valor que la restricción actual ya no permite —
-- y por eso fallaba al cerrar el último ítem pendiente de un pedido.
--
-- Ejecutar COMPLETO en Supabase → SQL Editor (idempotente)
-- Requiere haber corrido fase1, fase6, fase11 (pagos-por-consumo) y fase12.
-- ============================================================
